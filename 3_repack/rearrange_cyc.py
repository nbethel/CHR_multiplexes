...
for Nreps in range(4,7):
   # lline2='../../4_reloop_v2/two_helix_scaffolds_2mers/'+lline2
    ...
    fout.write('set sel1 [atomselect top "resid %d to %d and chain A and backbone"]\n'%((startID),(origLen)))
    fout.write('set sel2 [atomselect top "resid %d to %d and chain A and backbone"]\n'%((startID+origLen),(2*origLen)))
    fout.write('set transformation_matrix [measure fit $sel1 $sel2]\n')
    fout.write('set move_sel [atomselect top "resid %d to %d and chain A"]\n'%(startID,origLen))
